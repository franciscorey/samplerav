...
function setCueCreationControlsEnabled(enabled) {
  [dom.cueNewButton, dom.cueDivide2, dom.cueDivide4, dom.cueDivide8].forEach(btn => {
...
function renderTimeline() {
  ...
  if (!video) {
    ...
    setCueCreationControlsEnabled(false);
    return;
  }

  setCueCreationControlsEnabled(!!video.duration && video.duration > 0);
...
function addNewCue() {
  ...
  if (!video.duration || video.duration <= 0) {
    setStatus("DURACIÓN DEL VIDEO AÚN DESCONOCIDA — ESPERÁ A QUE CARGUE", false);
    return;
  }
